          ...
          <div className="row mbxl">
            <div className="col-12">
              <div>
                ...
                <input id="your-name" type="text" className="mls" value={name} onChange={(event) => setName(event.target.value)}/>
              </div>
              <button onClick={submit} disabled={remainingNames.length > 0 || name.length == 0}>Submit</button>
            </div>
          </div>
            ...
            <div className="col-7">
              ...
              {props.songs.map(song => (
                <div className="mbl" key={song.title}>
                  ...
                  <p>{song.artist}</p>
                  <p className="mld"><i>{song.comment}</i></p>
                  <select className="pas width-50" onChange={(event) => selectName(event, song.title)}>
                    {guesses[song.title]
                      ? <option value={guesses[song.title]} selected>{guesses[song.title]}</option>
                      : <option value="" hidden disabled selected>-- select --</option>}
                    {remainingNames.map(name => (
                      <option value={name} key={song.title + name}>{name}</option>
                    ))}
                  </select>
